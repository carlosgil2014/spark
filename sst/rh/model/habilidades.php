...
//Facilidad de palabra,Ejecucion de servicio en PDV
			...
			if($resultado){
		  		if($this->conexion->affected_rows === 1)
		  			$consulta2 = "INSERT INTO spartodo_rh.habilidadesHistorial (`idHabilidad`, `fecha`, `usuario`, `nombreHabilidad`) VALUES ($idHabilidad,'$hoy',$usuario,'$habilidad');";
	...
	public function actualizar($id,$habilidad){
		$id = $this->conexion -> real_escape_string(strip_tags(stripslashes(trim($id))));
		$habilidad = $this->conexion -> real_escape_string(strip_tags(stripslashes(trim($habilidad))));
		$errores = 0;
		$errorResultado = "";
