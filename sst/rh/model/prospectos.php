...
class prospectos
{
	...
	public function verMatch($vacante,$solicitudEmpleo){
		...
		if($resultado){
			...
			if($tmpPuntaje >= 5){ //mayor de la mitad de puntaje
	...
	public function prospectos($vacante){
		...
		$consulta="SELECT se.idSolicitudEmpleo,se.nombresDatosPersonales,se.apellidoPaternoDatosPersonales,se.apellidoMaternoDatosPersonales,se.puesto,se.sueldo,se.diasTrabajados,se.idPromocion,se.edad,se.sexo,ae.ultimoGradoEstudios,se.experienciaPuesto,ae.habilidades,ae.paquetesLenguajes,ae.conocimientosEspecificos,es.escolaridad FROM spartodo_rh.tblSolicitudEmpleo se left join spartodo_rh.tblAcademiaExperiencia ae ON ae.idSolicitudEmpleo=se.idSolicitudEmpleo LEFT JOIN spartodo_rh.tblEscolaridad es on es.idEscolaridad=ae.ultimoGradoEstudios WHERE se.puesto = '$puesto' and se.rfc not in (SELECT empleados_rfc from spartodo_spar_bd.spar_empleados) limit 10";
		$resultado = $this->conexion->query($consulta);
		// echo $consulta;
		if($resultado){
			while ($filaTmp = $resultado->fetch_assoc()) {
