...
class usuariosMA
	{
		...
		public function actualizar($datos,$idUsuario){
			...
			if($errores === 0){
				$consulta = "UPDATE tblUsuarios u SET u.usuarios_usuario = '$usuario', u.usuarios_contrasena = SHA1('$contrasena') WHERE u.usuarios_id = '$idUsuario' AND 0 = (SELECT COUNT(*) FROM (SELECT * FROM (SELECT * FROM tblUsuarios) AS u2 WHERE u2.usuarios_usuario = '$usuario' AND u2.usuarios_id != '$idUsuario') AS count); ";
  				$resultado = $this->conexion -> query($consulta);
				if($resultado){
			  		if($this->conexion->affected_rows === 1){
			  			return "OK";
