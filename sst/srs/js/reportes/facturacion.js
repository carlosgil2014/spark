...
$(document).on("click",".campos",function() { // Ver los campos seleccionados
  ...
  if(tmp){
    $(this).closest("tr").find('td:eq('+celda+')').attr("class","text-center warning");
    $(this).closest("tr").find('td:eq('+campo+')').attr("class","text-center warning");
  }
  else{
...
$(document).on("click", "#btnBuscar", function()
{
  ...
  if(idClientes.length > 0)
    $(".loader").fadeIn("slow", function(){
      $.post( "../controller/crudReportes.php?accion=tablaFacturas",{idClientes : idClientes, fechaInicial : fechaInicial, fechaFinal : fechaFinal},function(data)
        {
...
$(document).on("click", "#btnExportar", function()
{
  ...
  if(idClientes.length > 0)
    $(".loader").fadeIn("slow", function(){
      $.ajax({
        method: "POST",
        url:   "crudReportes.php?accion=reporteFacturacion",
