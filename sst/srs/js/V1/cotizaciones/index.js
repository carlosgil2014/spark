...
$(function()
{
    ...
    var num = document.getElementById("IdtxtTotal").innerText;
    var num1 = num.replace(/[$,]/g,"");
    nn(num1);   
    $("#IdcmbClientes").selectpicker(parametros);
...
{ 
  ...
  $.ajax({
    data: {idCliente:cliente[1],tipoServicio:tipoServicio},
    url: "index.php?accion=listarDatos&nombreCliente="+cliente[0]+"&idCliente="+cliente[1]+"&fechaInicial="+$("#fechaIn").val()+"&fechaFinal="+fechaFin+"&servicio="+$("#IdtxtServicio").val()+"&tipoPlan="+$("#IdcmbTipoPlan").val()+"&tipoServicio="+tipoServicio,
...
function cargarConceptos(){
  ...
  if(cliente[1] != "" && tipoServicio != null)
  {
    $(".loader").fadeIn("slow", function(){
      $.ajax({
        data: {idCliente:cliente[1],tipoServicio:tipoServicio},
        url:'../conceptos/index.php?accion=cargarConceptos',
        type:  'post',
        success:  function (data) {
          $("#IdcmbTipoConcepto").html(data);
          $("#IdcmbTipoConcepto").selectpicker(parametros);
          $("#IdcmbTipoConcepto").selectpicker("refresh");
        },
      });
    });
    $(".loader").fadeOut("slow");
...
$("#IdcmbTipoConcepto").change(function()
{
    ...
    if (datoSeleccionado!="Seleccione") 
    {
      ...
      $(".loader").fadeIn("slow", function(){
      $.ajax({
        data: {concepto : datoSeleccionado[0] },
        url:'../conceptos/index.php?accion=preciosFijos',
        type:  'post',
        success:  function (data) {
          if(data==0){
            $("#IdtxtPUnitario").val("");
...
function nn(n)
{
 ...
 for (var i=0; i<m.length; i+=3)
  ...
  if(n!="NaN"){
  document.getElementById("IdtxtCantidadLetra").innerText=t;}
  else{
  document.getElementById("IdtxtCantidadLetra").innerText="";}
