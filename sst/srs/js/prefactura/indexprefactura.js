...
$(document).on("click",".imgDetallePf",function()
{ 
    var idPrefactura = $(this).attr("value");
    $.post( "../controller/crudPrefacturas.php?accion=modalIndexPf",{  idPrefactura : idPrefactura },function(data)
    {
        $("#IdModaldetallePf").html(data);
  ...
  var idPrefactura = $(this).val();
    ...
    if(tipoPrefactura == "cd"){
      $(".editablesPf").each(function() {
        ...
        $.post( "crudPrefacturas.php?accion=maximosConceptos",{idCotOsConcepto : idCotOsConcepto , tabla : tabla, idPrefactura : idPrefactura},function(data)
        { 
            //alert(data);
...
$(document).on("click", "#guardarCambiosPrefactura", function() {
  ...
  if(comprobartablaPf())
  {
    $.post( "crudPrefacturas.php?accion=actualizarPrefactura",{idPrefactura : idPrefactura , detalle : detalle, idclienteprefactura: idClientePrefactura, descuento : descuento, motivodescuento : motivodescuento, total : total,estado:estado},function(data)
    { 
      $(".editablesPf").each(function() {
        var datosConcepto = {}; 
        ...
        datosConcepto["subtotal"] = (datosConcepto["cantidad"] * datosConcepto["precioUnitario"] * comision).toFixed(2);
         $.post( "crudPrefacturas.php?accion=actualizarPrefacturaConcepto",{datosConcepto : datosConcepto},function(data)
          { 
...
$(document).on('click', "button#asignarCfdi", function() { 
  $.post( "crudPrefacturas.php?accion=asignarCfdi",{cfdi : $('#cfdiPrefactura').val() , idPrefactura : $(this).val() , fechaCfdi: $('#fechaCfdi').val()},function(data){
         $("#divCfdi").html(data); 
  });
});