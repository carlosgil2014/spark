...
if(isset($_GET["accion"]))
{
	switch ($_GET["accion"]) 
	{
		case 'listarDatos':
			...
			$user = new usuarios();
			$nombre = $user -> nombreUsuario($usuario);
		...
		case 'modalIndexCot':
			...
			require_once('../model/cotizaciones.php');
			$cotizacion = new cotizaciones();
			$datosModalCotizacion=$cotizacion->datosCotizacionEspecifica($_POST["idCotizacion"]);
			$fechaInicio = new DateTime($datosModalCotizacion["fechaInicial"]);  
    		$fechaFin = new DateTime($datosModalCotizacion["fechaFinal"]);
    		require_once("../model/usuarios.php");
			$usuario= new usuarios();
			$name_usuario=$usuario->nombreUsuario($datosModalCotizacion["realizo"]);
			$filaConceptosCot=$cotizacion->listarConceptosCot($_POST["idCotizacion"]);
			$sumaConceptos=$cotizacion->sumaConceptosCot($_POST["idCotizacion"]);
			$folioCotizacion = "COT-".$datosModalCotizacion['clave_cliente']."-".$datosModalCotizacion['anio']."-".$datosModalCotizacion['ncotizacion'];

			require_once("../model/permisos.php");
			$permisos = new permisos();
			$permisosUsuario = $permisos->verificarPermiso($_SESSION['srs_usuario'],1);
			// if(is_array($filaConceptosCot))
			// {
				// foreach($filaConceptosCot as $filaConceptos)
		  //       {
		        	// $folio = "";
		        	// $tipoProceso = $cotizacion->tipoProceso($filaConceptos["idcotconcepto"],$folioCotizacion);
		        	
		     //    	foreach($tipoProceso as $proceso)
		     //    	{
			    //     	if($proceso["oS"] == $filaConceptos["idcotconcepto"])
			    //     	{
			    //     		$folio .= $folioCotizacion."<span class=\"glyphicon glyphicon-arrow-right\" aria-hidden=\"true\"></span>".$proceso["folio2"];
			    //     		if($proceso["folio3"] != "")
			    //     		$folio .= "<span class=\"glyphicon glyphicon-arrow-right\" aria-hidden=\"true\"></span>".$proceso["folio3"]."<br>";
			    //     	}
			    //     	else if($proceso["pf"] == $filaConceptos["idcotconcepto"])
			    //     	{
			    //     		$folio .= $folioCotizacion."<span class=\"glyphicon glyphicon-arrow-right\" aria-hidden=\"true\"></span>".$proceso["folio3"];
			    //     		if($proceso["folio2"] != "")
			    //     		$folio .= "<span class=\"glyphicon glyphicon-arrow-right\" aria-hidden=\"true\"></span>".$proceso["folio2"]."<br>";
			    //     	}
			    //     }
			    //     $origen[] = $folio;
		    	// }
		...
		case 'guardarModificacionModal':
			...
			header("location:crudCotizaciones.php?accion=modificarCotizacion&idCotizacion=".$_POST["idCotizacionModal"]);
				...
				//echo "hubo un error al guardar la modificación";
				print_r($concepto);
			break;
		...
		case 'guardarModificacionCot':
			...
			for($pos=1;$pos<=$_SESSION['cont'];$pos++)
				...
				//echo $resultadoConceptosCotizacion;
				for($i=1;$i<=$_SESSION['cont'];$i++) 
			    {     
			                        
			        
			        /*unset($_SESSION['cmbClientesMod'][$i]);
					unset($_SESSION['txtServicio'][$i]);*/
					unset($_SESSION['cmbTipoPlanMod'][$i]);
					unset($_SESSION['cmbTipoServicioMod'][$i]);
					unset($_SESSION['txtCantidadMod'][$i]);
