	...
	class devoluciones{
		...
		public function __construct() {
   		...
   		public function guardar($datosPrefactura, $idPfConcepto, $devolucion, $servicio){
   			...
   			$realizo = $this->conexion -> real_escape_string(strip_tags(stripslashes(trim($_SESSION["spar_usuario"]))));
			...
			if($resultado){
				if($this->conexion->affected_rows === 1){
					...
					$consulta = "INSERT INTO spartodo_srs.tblordenesconceptos (idorden, idpfconcepto, idconcepto, concepto, tipoplan, tiposervicio, cantidad, precioUnitario, comision, total,descripcion) (SELECT '$idDevolucion', idprefacturaconcepto, idconcepto, 'Devolución', tipoplan, tiposervicio, '1', '$devolucion', '0.00', '$devolucion','' FROM spartodo_srs.tblprefacturasconceptos WHERE idprefacturaconcepto = '$idPfConcepto' AND idprefactura = '$idPrefactura'); ";
					// echo $consulta;
					$resultado = $this->conexion->query($consulta);
					if($resultado){
						if($this->conexion->affected_rows === 1){
		...
		public function nDevolucion($clave){
			...
			$año = date("y");
  			$consulta = "SELECT COALESCE(MAX(o.norden),0) as maxima FROM spartodo_srs.tblordenesdeservicio o LEFT OUTER JOIN tblClientes cl ON o.idcliente = cl.idclientes WHERE cl.nombreComercial = '$clave' AND o.estado like '%Devolucion%' AND o.anio = '$año'";
