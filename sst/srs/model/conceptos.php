<?php 
	class conceptos{
   		...
   		public function datosConcepto($idConcepto)
		{
			...
			$consulta = "SELECT c.nombreRubro, c.id,c.precio,c.categoria FROM spartodo_srs.tblconceptos c WHERE id = '".$idConcepto."'";
		...
		{
			$idConcepto = $this->conexion->real_escape_string(strip_tags(stripslashes(trim($idConcepto))));
			$query = "SELECT precio FROM spartodo_srs.tblconceptos WHERE id = '$idConcepto'";
