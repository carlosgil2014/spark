...
class estados
{
	...
	public function listar(){
		$datos = array();
		$consulta="SELECT `idestado`, `idpais`, `nombre`, `region` FROM `tblEstados` order by nombre";
	...
	public function informacion($id){
		$id = $this->conexion -> real_escape_string(strip_tags(stripslashes(trim($id))));
			$consulta="SELECT `idestado`, `idpais`, `nombre`, `region` FROM `tblEstados` where idestado=$id";
		$resultado = $this->conexion->query($consulta);
		if($resultado){
