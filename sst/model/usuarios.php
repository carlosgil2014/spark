	...
	public function buscarEmpleado($buscar)
    {
      ...
      public function informEmpleado($idEmpleado){
			$idEmpleado = $this->conexion -> real_escape_string(strip_tags(stripslashes(trim($idEmpleado))));
   			$consulta="SELECT e.empleados_id,CONCAT(e.empleados_apellido_paterno,' ',e.empleados_apellido_materno,' ',e.empleados_nombres) AS nombre, e.empleados_correo,p.puesto FROM spar_empleados e LEFT join tblPuestos p on e.empleados_puesto=p.idPuesto WHERE e.empleados_id = $idEmpleado";
   			// echo $consulta;
			$resultado = $this->conexion->query($consulta);
			if($resultado){
				return $resultado->fetch_assoc();
