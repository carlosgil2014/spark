...
class codigoPostal
{
	...
	public function listar(){
		$datos = array();
		$consulta="SELECT cp.idcp,cp.cp,cp.asentamiento,cp.tipo,cp.delegacion,cp.estado,cp.ciudad FROM tblCP cp";
	...
	public function informacion($id){
		$id = $this->conexion -> real_escape_string(strip_tags(stripslashes(trim($id))));
			$consulta="SELECT  cp.idcp,cp.cp,cp.asentamiento,cp.tipo,cp.delegacion,cp.estado,cp.ciudad FROM tblCP cp WHERE `cp.idcp`=$id";
		$resultado = $this->conexion->query($consulta);
		if($resultado){
