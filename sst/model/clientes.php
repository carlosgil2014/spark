...
class clientes
	{
		...
		{ 
   		...
   		public function listar(){
   			$datos = array();
   			$consulta="SELECT idclientes, if(razonSocial='',CONCAT(nombres,' ',apellidoPaterno,' ',apellidoMaterno),razonSocial) AS razonSocial, nombreComercial, rfc FROM tblClientes c ORDER BY nombreComercial";
			// echo $consulta;
   		...
   		public function usuariosClientes($idUsuario){
			$idUsuario = $this->conexion -> real_escape_string(strip_tags(stripslashes(trim($idUsuario))));
   			$datos = array();
   			$consulta="SELECT c.idclientes, c.nombreComercial FROM tblClientes c LEFT JOIN tblUsuariosClientes uc ON c.idclientes = uc.idCliente WHERE uc.idUsuario = '$idUsuario'";
			// echo $consulta;
			$resultado = $this->conexion->query($consulta);
