...
class proveedores
	{
		...
		public function __construct() 
		{
   		...
   		public function listar(){
   			$datos = array();
   			$consulta="SELECT idproveedor, if(razonSocial='',CONCAT(nombres,' ',apellidoPaterno,' ',apellidoMaterno),razonSocial) AS razonSocial, nombreComercial, estado ,telefonoContactoPrincipal, b.nombre as bancoProveedor, noCuentaProveedor, clabeProveedor,rfc,cp FROM tblProveedores p LEFT JOIN tblBancos b ON p.bancoProveedor = b.idbanco ";
	...
	public function rfc($rfc){
		$rfc = $this->conexion -> real_escape_string(strip_tags(stripslashes(trim($rfc))));
			$consulta="SELECT p.rfc FROM tblProveedores p where p.rfc='$rfc'";
		$resultado = $this->conexion->query($consulta);
		if($resultado){
			return $resultado->fetch_assoc();
		}
